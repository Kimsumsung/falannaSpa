...
{	
	...
	{
		$sql = "INSERT INTO ". $this->info->massageTable ."(topic,description,userid,status) VALUES ('".$topic."','".$description."','".$userid."','Active')";
	...
	function getNews()
	{
		$sql = "SELECT * FROM ". $this->info->massageTable ." ORDER BY newsid ASC";
		$result = $this->db->excuteQuery($sql);
	...
	{
		$sql = "SELECT * FROM ". $this->info->massageTable ." ORDER BY newsid DESC";
		$result = $this->db->excuteQuery($sql);
	...
	{
		$sql = "DELETE FROM ". $this->info->massageTable ." WHERE newsid='". $newsid ."'";
	...
	{
		$sql = "SELECT * FROM ". $this->info->massageTable ." WHERE newsid='". $newsid ."'";
		$result = $this->db->excuteQuery($sql);

		if (!$result || mysql_num_rows($result) == 0)
			return false; // news not existed
		
		$sql = "UPDATE ". $this->info->massageTable ." SET topic='". $topic ."', description='". $description ."', status='". $status ."' WHERE newsid='". $newsid ."'";
		$result = $this->db->excuteQuery($sql);
...
if(isset($_POST['fetchnewsid']) && !empty($_POST['fetchnewsid'])) {
	...
	$sql = "SELECT * FROM ". $info->massageTable ." WHERE newsid='". $_POST['fetchnewsid'] ."'";
