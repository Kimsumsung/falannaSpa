...
{	
	...
	{
		$sql = "INSERT INTO ". $this->info->spaTable ."(topic,description,userid,status) VALUES ('".$topic."','".$description."','".$userid."','Active')";
	...
	function getNews()
	{
		$sql = "SELECT * FROM ". $this->info->spaTable ." ORDER BY newsid ASC";
		$result = $this->db->excuteQuery($sql);
	...
	{
		$sql = "SELECT * FROM ". $this->info->spaTable ." ORDER BY newsid DESC";
		$result = $this->db->excuteQuery($sql);
	...
	{
		$sql = "DELETE FROM ". $this->info->spaTable ." WHERE newsid='". $newsid ."'";
	...
	{
		$sql = "SELECT * FROM ". $this->info->spaTable ." WHERE newsid='". $newsid ."'";
		$result = $this->db->excuteQuery($sql);

		if (!$result || mysql_num_rows($result) == 0)
			return false; // news not existed
		
		$sql = "UPDATE ". $this->info->spaTable ." SET topic='". $topic ."', description='". $description ."', status='". $status ."' WHERE newsid='". $newsid ."'";
		$result = $this->db->excuteQuery($sql);
...
if(isset($_POST['fetchnewsid']) && !empty($_POST['fetchnewsid'])) {
	...
	$sql = "SELECT * FROM ". $info->spaTable ." WHERE newsid='". $_POST['fetchnewsid'] ."'";
